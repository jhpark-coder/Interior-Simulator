...
function checkClosetDoorCollision(
  ...
  for (const dz of zLines) {
    for (let s = 1; s <= 10; s++) {
      ...
      if (wx < wallMargin || wx > roomWidth - wallMargin || wz < wallMargin || wz > roomDepth - wallMargin) {
        return true;
      }
      ...
      for (const f of furniture) {
        ...
        if (Math.abs(flx) <= f.width / 2 + furnitureMargin && Math.abs(flz) <= f.depth / 2 + furnitureMargin) {
          return true;
        }
      }
    }
...
export function DisplayCabinetMesh({ item, color }: { item: FurnitureItem; color: string }) {
  ...
  useFrame(() => {
    ...
    const isOpening = Math.abs(nextAngle) > Math.abs(doorAngleRef.current) + 0.0001;

    if (isOpening) {
      const cosItem = Math.cos(itemRotation);
      const sinItem = Math.sin(itemRotation);

      if (checkClosetDoorCollision(nextAngle, -panelW / 2, depth / 2 - frameSize / 2, panelW, 1, panelThick / 2, itemCenterX, itemCenterZ, cosItem, sinItem, room.width, room.height, furniture, item.id)) {
        let lo = doorAngleRef.current;
        let hi = nextAngle;
        for (let i = 0; i < 5; i++) {
          const mid = (lo + hi) / 2;
          if (checkClosetDoorCollision(mid, -panelW / 2, depth / 2 - frameSize / 2, panelW, 1, panelThick / 2, itemCenterX, itemCenterZ, cosItem, sinItem, room.width, room.height, furniture, item.id)) {
            hi = mid;
          } else {
            lo = mid;
          }
  ...
  return (
    <>
      ...
      {[1, 2, 3].map((i) => (
        <mesh key={`shelf-${i}`} position={[0, baseH + shelfSpacing * i, 0]}>
          <boxGeometry args={[width - frameSize * 2 - 4, shelfThick, depth - frameSize * 2 - 4]} />
          <meshPhysicalMaterial color="#d4eaf5" metalness={0.1} roughness={0.15} clearcoat={1} clearcoatRoughness={0.05} />
        </mesh>
      ))}
      <mesh position={[0, innerCenterY, -depth / 2 + frameSize / 2]}>
        <boxGeometry args={[panelW, panelH, panelThick]} />
        <meshPhysicalMaterial color="#cce8f4" transmission={0.85} roughness={0.05} thickness={panelThick} />
      </mesh>
      <mesh position={[-width / 2 + frameSize / 2, innerCenterY, 0]}>
        <boxGeometry args={[panelThick, panelH, depth - frameSize * 2 - 4]} />
        <meshPhysicalMaterial color="#cce8f4" transmission={0.85} roughness={0.05} thickness={panelThick} />
      </mesh>
      <mesh position={[width / 2 - frameSize / 2, innerCenterY, 0]}>
        <boxGeometry args={[panelThick, panelH, depth - frameSize * 2 - 4]} />
      ...
      <group position={[-panelW / 2, innerCenterY, depth / 2 - frameSize / 2]}>
        <group ref={doorGroupRef}>
          <mesh position={[panelW / 2, 0, 0]} onClick={toggleDoor}>
            <boxGeometry args={[panelW, panelH, panelThick + handleR * 4]} />
          ...
          <mesh position={[panelW - 25, 0, panelThick / 2 + handleR]} castShadow>
...
export function ClosetMesh({ item, color }: { item: FurnitureItem; color: string }) {
  const room = useSimulatorStore((s) => s.room);
  const furniture = useSimulatorStore((s) => s.furniture);
  ...
  useFrame(() => {
    ...
    if (leftDoorGroupRef.current) {
      ...
      if (Math.abs(diff) > 0.001) {
        ...
        const isOpening = Math.abs(nextAngle) > Math.abs(leftAngleRef.current) + 0.0001;

        if (isOpening && checkClosetDoorCollision(nextAngle, -innerW / 2, depth / 2, doorW, 1, doorT / 2, itemCenterX, itemCenterZ, cosItem, sinItem, room.width, room.height, furniture, item.id)) {
          let lo = leftAngleRef.current;
          let hi = nextAngle;
          for (let i = 0; i < 5; i++) {
            const mid = (lo + hi) / 2;
            if (checkClosetDoorCollision(mid, -innerW / 2, depth / 2, doorW, 1, doorT / 2, itemCenterX, itemCenterZ, cosItem, sinItem, room.width, room.height, furniture, item.id)) {
              hi = mid;
            } else {
              lo = mid;
            }
    ...
    if (rightDoorGroupRef.current) {
      ...
      if (Math.abs(diff) > 0.001) {
        ...
        const isOpening = Math.abs(nextAngle) > Math.abs(rightAngleRef.current) + 0.0001;
    ...
    if (drawerGroupRef.current) {
      ...
      if (Math.abs(diff) > 0.5) {
        ...
        if (isOpening && checkClosetDrawerCollision(nextOffset, depth / 2 + raisedT + knobR, innerW - 4, itemCenterX, itemCenterZ, cosItem, sinItem, room.width, room.height, furniture, item.id)) {
          let lo = drawerOffsetRef.current;
          let hi = nextOffset;
          for (let i = 0; i < 5; i++) {
            const mid = (lo + hi) / 2;
            if (checkClosetDrawerCollision(mid, depth / 2 + raisedT + knobR, innerW - 4, itemCenterX, itemCenterZ, cosItem, sinItem, room.width, room.height, furniture, item.id)) {
              hi = mid;
            } else {
              lo = mid;
            }
  ...
  const toggleLeftDoor = useCallback((e: ThreeEvent<MouseEvent>) => { e.stopPropagation(); setLeftDoorOpen((p) => !p); }, []);
  const toggleRightDoor = useCallback((e: ThreeEvent<MouseEvent>) => { e.stopPropagation(); setRightDoorOpen((p) => !p); }, []);
  const toggleDrawer = useCallback((e: ThreeEvent<MouseEvent>) => { e.stopPropagation(); setDrawerOpen((p) => !p); }, []);

  return (
    <>
      {([ [-width / 2 + legInset + legDiam / 2, -depth / 2 + legInset + legDiam / 2], [width / 2 - legInset - legDiam / 2, -depth / 2 + legInset + legDiam / 2], [-width / 2 + legInset + legDiam / 2, depth / 2 - legInset - legDiam / 2], [width / 2 - legInset - legDiam / 2, depth / 2 - legInset - legDiam / 2] ] as [number, number][]).map(([x, z], i) => (
        <mesh key={`leg-${i}`} position={[x, legH / 2, z]}>
          <cylinderGeometry args={[legDiam / 2, legDiam * 0.35, legH, 8]} />
          <meshStandardMaterial color={color} />
        </mesh>
      ))}
      <mesh position={[-width / 2 + sideT / 2, legH + (height - legH) / 2, 0]} castShadow receiveShadow>
      ...
      <mesh position={[0, legH + (height - legH) / 2, -depth / 2 + backT / 2]}>
      ...
      <group ref={drawerGroupRef}>
        <mesh position={[0, drawerBottom + drawerH / 2, depth / 2 - doorT / 2]} onClick={toggleDrawer} castShadow>
          <boxGeometry args={[innerW - 4, drawerH - 4, doorT]} />
          <meshStandardMaterial color={color} />
        </mesh>
        <mesh position={[0, drawerBottom + drawerH / 2, depth / 2 + raisedT / 2]} onClick={toggleDrawer}>
          <boxGeometry args={[innerW * 0.8, drawerH * 0.55, raisedT]} />
          <meshStandardMaterial color={color} />
        </mesh>
        <mesh position={[0, drawerBottom + drawerH / 2, depth / 2 + raisedT + knobR * 0.5]} onClick={toggleDrawer}>
        ...
        <mesh position={[-(innerW / 2 - 6), drawerBottom + drawerH / 2, -(backT + doorT) / 2]}>
      ...
      <group position={[-innerW / 2, doorBottom, depth / 2]}>
        <group ref={leftDoorGroupRef}>
          <mesh position={[doorW / 2, doorSectionH / 2, 0]} onClick={toggleLeftDoor}>
            <boxGeometry args={[doorW, doorSectionH, doorT + raisedT * 2 + knobR * 2]} />
            <meshBasicMaterial transparent opacity={0} depthWrite={false} />
          </mesh>
          <mesh position={[doorW / 2, doorSectionH / 2, -doorT / 2]} castShadow>
            <boxGeometry args={[doorW - 2, doorSectionH - 2, doorT]} />
            <meshStandardMaterial color={color} />
          </mesh>
          <mesh position={[doorW / 2, doorSectionH - panelMargin - upperPanelH / 2, raisedT / 2]}>
            <boxGeometry args={[panelDetailW, upperPanelH, raisedT]} />
            <meshStandardMaterial color={color} />
          </mesh>
          <mesh position={[doorW / 2, panelMargin + lowerPanelH / 2, raisedT / 2]}>
            <boxGeometry args={[panelDetailW, lowerPanelH, raisedT]} />
            <meshStandardMaterial color={color} />
          </mesh>
          <mesh position={[doorW - panelMargin, doorSectionH / 2, raisedT + knobR * 0.5]}>
            <sphereGeometry args={[knobR, 12, 8]} />
            <meshPhongMaterial color="#aaaaaa" specular="#ffffff" shininess={120} />
          </mesh>
        </group>
      ...
      <group position={[innerW / 2, doorBottom, depth / 2]}>
        <group ref={rightDoorGroupRef}>
          <mesh position={[-doorW / 2, doorSectionH / 2, 0]} onClick={toggleRightDoor}>
            <boxGeometry args={[doorW, doorSectionH, doorT + raisedT * 2 + knobR * 2]} />
            <meshBasicMaterial transparent opacity={0} depthWrite={false} />
          </mesh>
          <mesh position={[-doorW / 2, doorSectionH / 2, -doorT / 2]} castShadow>
